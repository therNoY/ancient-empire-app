<template>
	<div id="mapEdit">
		...
		<div class="unit" v-if="flushUnitFlag">
			...
			<div v-for="(unit, key) in this.initMapInfo.unit_mes_list" v-bind:key="key">
				<img class="select_unit" v-if="unit.id == selectUnit.id" :src="$appHelper.getUnitImg(unit.id, color)"
					@click="getUnit(unit)" />
				<img class="un_select_unit" v-else :src="$appHelper.getUnitImg(unit.id, color)"
					@click="getUnit(unit)" />
			</div>
				...
				<el-main>
					<div class="map_div" :style="mapStyle">
						<div class="map" :style="mapWidthStyle">
							<img class="unit_img" v-for="(unit, index) in unitList" v-bind:key="index" :src="
                  $appHelper.getUnitImg(
                    unit.id ? unit.id : unit.type_id,
                    unit.color
                  )
                " :title="getUnitTitle(unit)" :style="{
                  top: $appHelper.getUnitPosition(unit.row),
                  left: $appHelper.getUnitPosition(unit.column),
                }" @click="clickEditMapUnit(unit, index)" />
							<region-view-list ref="regionViewList" :regions="maps" :row="mapRow" :column="mapColumn"
								showTitle @clickRegion="clickEditMapRegion" />
						</div>
					</div>
				</el-main>
				<el-footer class="opera_button">
					<ae-button-list :factor="70" :buttonList="buttonList" :clickAction="mapEditActionList"></ae-button-list>
				</el-footer>
			</el-container>
		</div>
		...
		<div class="region">
			...
			<div v-for="(region, index) in this.initMapInfo.region_mes" v-bind:key="index">
				<img class="select_unit" v-if="region.id == selectRegion.id"
					:src="$appHelper.getRegionImg(region.type, regionColor)" @click="getRegion(region)" />
				<img class="un_select_unit" v-else :src="$appHelper.getRegionImg(region.type, regionColor)"
		...
		<!--设置宽度弹出框 改变大小新建-->
		<ae-base-dialog class="size_dialog" title="设置新建地图大小" v-model="showSetNewMapInfo" :width="30">
		...
		<!--我的地图-->
		<ae-complex-dialog ref="myMap" v-model="showMapVisible" :showItem="showItem" showSearch title="我的地图"
			:initQueryDataGrid="queryFunction" :footerButtons="myMapEditButtonList" :width="40" page>
		</ae-complex-dialog>

		<template-select v-model="showSelectTemp" @onSelect="createNewMapByTemplateId"></template-select>

		<!--预览地图-->
		<map-preview v-model="previewVisible" :mapId="currPreviewMap.map_id"></map-preview>

		<!--用于保存或者发布-->
		<ae-complex-dialog v-model="showChangeMsg" title="保存地图" :formConfig="baseMapFormConfig"
			:dataObj="currentMapInfo" :footerButtons="changMapInfoButtons"></ae-complex-dialog>
	</div>
</template>
